...
import AlertForm from './AlertForm'; // Ensure the correct path to AlertForm
...
export default function Alerts() {
  ...
  const [alerts, setAlerts] = useState([
    { 
      ...
      mensaje: 'El tanque 1 está por debajo del 10%', 
      nivel: 'critica' 
    },
    { 
      id: 2,
      tipo: 'Mantenimiento', 
      mensaje: 'La bomba 2 necesita revisión', 
      nivel: 'advertencia' 
    },
    { 
      id: 3,
      tipo: 'Actualización', 
      mensaje: 'Se realizó una carga exitosa de combustible', 
      nivel: 'informativa' 
    },
  ]);
  ...
  const handleSubmit = () => {
    if (isEditing) {
      setAlerts(prev =>
        prev.map(alert =>
          alert.id === formData.id ? formData : alert
        )
      );
    } else {
  ...
  return (
    <>
            ...
            <Card className="bg-gradient-danger border-0 shadow">
              <CardBody className="py-3">
                <div className="text-white">
                  ...
                  <h2 className="mb-0">
                    {alerts.filter(a => a.nivel === 'critica').length}
                  </h2>
                </div>
              </CardBody>
            </Card>
            ...
            <Card className="bg-gradient-warning border-0 shadow">
              <CardBody className="py-3">
                <div className="text-white">
                  ...
                  <h2 className="mb-0">
                    {alerts.filter(a => a.nivel === 'advertencia').length}
                  </h2>
                </div>
              </CardBody>
            </Card>
            ...
            <Card className="bg-gradient-info border-0 shadow">
              <CardBody className="py-3">
                <div className="text-white">
                  ...
                  <h2 className="mb-0">
                    {alerts.filter(a => a.nivel === 'informativa').length}
                  </h2>
                </div>
              </CardBody>
            </Card>
      ...
      {/* Modal para el formulario (se mantiene igual) */}
      <AlertForm
        isOpen={modalForm}
        toggle={toggleForm}
